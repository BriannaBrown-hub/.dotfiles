...
function getCommandArguments(fileName) {
    ...
    if (extensionConfig.configFilePath !== '') {
        let found = [extensionConfig.configFilePath].concat((vscode.workspace.workspaceFolders || []).map((ws) => path.join(ws.uri.path, extensionConfig.configFilePath))).filter((p) => fs.existsSync(p));
        if (found.length == 0) {
