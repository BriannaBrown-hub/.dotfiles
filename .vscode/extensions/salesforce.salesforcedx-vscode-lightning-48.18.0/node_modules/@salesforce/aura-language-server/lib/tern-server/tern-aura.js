...
const tern_indexer_1 = require("./tern-indexer");
...
async function getLibraryIncludes(file, library) {
    var libFile = await tern_indexer_1.getLibFile(file, library);
    var inc = await tern_indexer_1.getLibIncludes(getFilename(libFile));
    var includes = [];
...
async function processIfLibrary(file, modules) {
    let lib = await tern_indexer_1.getLibraryForJS(file.name);
    // @ts-ignore
    let libs = lib ? [lib] : [];
    if (libs.length > 0) {
        ...
        var imps = await tern_indexer_1.getLibImports(libName, bn);
        ...
        if (imps) {
            ...
            for (var i = 0; i < imports.length; i++) {
                ...
                if (importedModule.indexOf(':') > -1) {
                    ...
                    var libfile = await tern_indexer_1.getLibFile(file.name, qn);
                    // @ts-ignore
                    if (!libfile)
                        continue;
            ...
            infer.withContext(server.cx, function () {
                for (var i = 0; i < libfilesResolved.length; i++) {
                    ...
                    if (lib.getType().hasProp(importedModule)) {
                        ...
                        /// been loaded.
                        lib.getType().on('addProp', function (pmType, prop, val) {
                            if (pname == prop) {
                                pmType.addType(val);
                            }
                        }.bind(this, pm));
...
async function processIfComponent(file, modules) {
    ...
    let cmp = await tern_indexer_1.getComponentForJS(file.name);
    // @ts-ignore
    let cmps = cmp ? [cmp] : [];
    if (cmps.length > 0) {
        _debug('Discover libs of: ' + cmps[0]);
        let ins = await tern_indexer_1.getCmpImports(getFilename(cmps[0]));
        let libs = [];
        // @ts-ignore
        for (let i = 0; i < ins.length; i++) {
            ...
            let libfile = await tern_indexer_1.getLibFile(file.name, library);
            libs.push(libfile);
        }
        ...
        server.request({
            ...
            if (shouldFilter) {
                result.completions = result.completions.filter(function (completion, index, array) {
                    var accepted = (completion.doc && completion.doc.indexOf('@platform') !== -1 && completion.origin === 'Aura') || completion.origin !== 'Aura';
