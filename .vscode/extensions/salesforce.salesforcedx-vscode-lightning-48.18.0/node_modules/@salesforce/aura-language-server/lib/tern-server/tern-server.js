...
async function startServer(rootPath, wsroot) {
    ...
    const config = Object.assign({}, defaultConfig, { defs,
        plugins, 
        // @ts-ignore 2345
        projectDir: rootPath, getFile(filename, callback) {
...
function* walkSync(dir) {
    const files = fs_2.readdirSync(dir);
    for (const file of files) {
        const pathToFile = path_1.default.join(dir, file);
        const isDirectory = fs_2.statSync(pathToFile).isDirectory();
        if (isDirectory) {
...
async function ternInit() {
    ...
    for (const file of found) {
        if (file.endsWith('.js')) {
            const data = fs_2.readFileSync(file, 'utf-8');
