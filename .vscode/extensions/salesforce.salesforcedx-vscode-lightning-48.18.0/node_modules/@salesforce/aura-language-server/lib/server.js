...
lightning_lsp_common_2.interceptConsoleLogger(connection);
...
connection.onInitialize(async (params) => {
    ...
    try {
        ...
        context = new lightning_lsp_common_2.WorkspaceContext(workspaceRoots);
        ...
        console.info('... language server started in ' + lightning_lsp_common_2.utils.elapsedMillis(startTime));
        return {
            capabilities: {
...
connection.onDidChangeWatchedFiles(async (change) => {
    ...
    try {
        const lwcIndexer = context.getIndexingProvider('lwc');
        lwcIndexer.handleWatchedFiles(context, change);
        if (lightning_lsp_common_2.utils.isAuraRootDirectoryCreated(context, changes)) {
            ...
            const startTime = process.hrtime();
            console.info('reindexed workspace in ' + lightning_lsp_common_2.utils.elapsedMillis(startTime) + ', directory was deleted:', changes);
            return;
        }
        else {
            let files = 0;
            for (const event of changes) {
                if (event.type === vscode_languageserver_1.FileChangeType.Deleted && lightning_lsp_common_2.utils.isAuraWatchedDirectory(context, event.uri)) {
