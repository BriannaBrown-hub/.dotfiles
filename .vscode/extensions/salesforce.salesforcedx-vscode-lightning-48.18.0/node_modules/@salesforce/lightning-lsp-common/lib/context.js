...
// @ts-ignore
const lodash_templatesettings_1 = __importDefault(require("lodash.templatesettings"));
// @ts-ignore
...
class WorkspaceContext {
    ...
    constructor(workspaceRoots) {
        ...
        this.AURA_EXTENSIONS = ['.cmp', '.app', '.design', '.evt', '.intf', '.auradoc', '.tokens'];
        this.workspaceRoots = typeof workspaceRoots === 'string' ? [path.resolve(workspaceRoots)] : workspaceRoots;
        this.type = shared_1.detectWorkspaceType(this.workspaceRoots);
    ...
    async isInsideModulesRoots(document) {
        const file = utils.toResolvedPath(document.uri);
        for (const ws of this.workspaceRoots) {
            if (utils.pathStartsWith(file, ws)) {
    ...
    async isFileInsideModulesRoots(file) {
        const namespaceRoots = await this.findNamespaceRootsUsingTypeCache();
    ...
    async isFileInsideAuraRoots(file) {
        const namespaceRoots = await this.findNamespaceRootsUsingTypeCache();
        ...
        switch (this.type) {
            ...
            case shared_1.WorkspaceType.CORE_ALL:
                ...
                for (const project of dirs) {
                    const modulesDir = path_1.join(this.workspaceRoots[0], project, 'modules');
            ...
            case shared_1.WorkspaceType.CORE_PARTIAL:
                for (const ws of this.workspaceRoots) {
                    const modulesDir = path_1.join(ws, 'modules');
                    if (await fs.pathExists(modulesDir)) {
                        list.push(modulesDir);
                    }
                }
    ...
    async writeTypings() {
        ...
        switch (this.type) {
            case shared_1.WorkspaceType.SFDX:
                typingsDir = path_1.join(this.workspaceRoots[0], '.sfdx', 'typings', 'lwc');
                break;
            case shared_1.WorkspaceType.CORE_PARTIAL:
                typingsDir = path_1.join(this.workspaceRoots[0], '..', '.vscode', 'typings', 'lwc');
                break;
            case shared_1.WorkspaceType.CORE_ALL:
                typingsDir = path_1.join(this.workspaceRoots[0], '.vscode', 'typings', 'lwc');
                break;
        ...
        if (typingsDir) {
            ...
            try {
                await fs.copy(path_1.join(resourceTypingsDir, 'lds.d.ts'), path_1.join(typingsDir, 'lds.d.ts'));
            }
            catch (ignore) {
                // ignore
            }
            try {
                await fs.copy(path_1.join(resourceTypingsDir, 'messageservice.d.ts'), path_1.join(typingsDir, 'messageservice.d.ts'));
            }
            catch (ignore) {
                // ignore
            }
            const dirs = await fs.readdir(path_1.join(resourceTypingsDir, 'copied'));
            for (const file of dirs) {
                try {
                    await fs.copy(path_1.join(resourceTypingsDir, 'copied', file), path_1.join(typingsDir, file));
                }
                catch (ignore) {
                    // ignore
                }
     ...
     */
    async writeJsconfigJson() {
        let jsConfigTemplate;
        let jsConfigContent;
        ...
        switch (this.type) {
            case shared_1.WorkspaceType.SFDX:
                ...
                const forceignore = path_1.join(this.workspaceRoots[0], '.forceignore');
                for (const modulesDir of modulesDirs) {
                    const jsConfigPath = path_1.join(modulesDir, 'jsconfig.json');
    ...
    async updateCoreSettings() {
        ...
        for (const ws of this.workspaceRoots) {
            await fs.ensureDir(path_1.join(ws, '.vscode'));
            this.updateConfigFile(path_1.join(ws, '.vscode', 'settings.json'), templateContent);
        }
    }
    async updateCoreCodeWorkspace() {
        const configBlt = await this.readConfigBlt();
        const variableMap = {
            eslint_node_path: await findCoreESLint(),
            p4_port: configBlt['p4.port'],
    ...
    async readConfigBlt() {
        const isMain = this.workspaceRoots[0].indexOf(path_1.join('main', 'core')) !== -1;
        let relativeBltDir = isMain ? path_1.join('..', '..', '..') : path_1.join('..', '..', '..', '..');
        if (this.type === shared_1.WorkspaceType.CORE_PARTIAL) {
            relativeBltDir = path_1.join(relativeBltDir, '..');
        }
        const configBltContent = await fs.readFile(path_1.join(this.workspaceRoots[0], relativeBltDir, 'config.blt'), 'utf8');
        return properties_1.parse(configBltContent);
    }
    processTemplate(templateString, variableMap) {
        lodash_templatesettings_1.default.interpolate = /\${([\s\S]+?)}/g;
        const compiled = lodash_template_1.default(templateString);
    ...
    async updateForceIgnoreFile(ignoreFile) {
     ...
     */
    async findNamespaceRootsUsingType() {
        const roots = {
            lwc: [],
            aura: [],
        };
        switch (this.type) {
            case shared_1.WorkspaceType.SFDX:
                ...
                for (const pkg of packageDirectories) {
                    const pkgDir = path_1.join(this.workspaceRoots[0], pkg.path);
            ...
            case shared_1.WorkspaceType.CORE_ALL:
                ...
                for (const project of await fs.readdir(this.workspaceRoots[0])) {
                    ...
                    const auraDir = path_1.join(this.workspaceRoots[0], project, 'components');
            ...
            case shared_1.WorkspaceType.CORE_PARTIAL:
                ...
                for (const ws of this.workspaceRoots) {
                    const modulesDir = path_1.join(ws, 'modules');
                    if (await fs.pathExists(modulesDir)) {
                        const subroots = await findNamespaceRoots(path_1.join(ws, 'modules'), 2);
                        roots.lwc.push(...subroots.lwc);
                    }
                    const auraDir = path_1.join(ws, 'components');
                    if (await fs.pathExists(auraDir)) {
                        const subroots = await findNamespaceRoots(path_1.join(ws, 'components'), 2);
                        roots.aura.push(...subroots.aura);
                    }
                }
                return roots;
            case shared_1.WorkspaceType.STANDARD:
            case shared_1.WorkspaceType.STANDARD_LWC:
            case shared_1.WorkspaceType.MONOREPO:
            case shared_1.WorkspaceType.UNKNOWN: {
                let depth = 6;
                if (this.type === shared_1.WorkspaceType.MONOREPO) {
                    depth += 2;
                }
...
async function readSfdxProjectConfig(root) {
    try {
        return JSON.parse(await fs.readFile(shared_1.getSfdxProjectFile(root), 'utf8'));
    }
    catch (e) {
        throw new Error(`Sfdx project file seems invalid. Unable to parse ${shared_1.getSfdxProjectFile(root)}. ${e.message}`);
    }
}
function getSfdxPackageDirs(sfdxProjectConfig) {
 ...
 */
async function findNamespaceRoots(root, maxDepth = 5) {
    const roots = {
        lwc: [],
        aura: [],
    };
    function isModuleRoot(subdirs) {
    ...
    async function isAuraRoot(subdirs) {
        for (const subdir of subdirs) {
            // Is a root if any subdir matches a name/name.js with name.js being a module
            const basename = path.basename(subdir);
    ...
    async function traverse(candidate, depth) {
        if (--depth < 0) {
            return;
        }
 ...
 */
async function findModulesIn(namespaceRoot) {
    const files = [];
    const subdirs = await findSubdirectories(namespaceRoot);
    for (const subdir of subdirs) {
        const basename = path.basename(subdir);
        const modulePath = path.join(subdir, basename + '.js');
        if ((await fs.pathExists(modulePath)) && index_1.componentUtil.isJSComponent(modulePath)) {
            // TODO: check contents for: from 'lwc'?
 ...
 * @return list of .js modules inside namespaceRoot folder
 */
async function findAuraMarkupIn(namespaceRoot) {
    // const files: string[] = [];
    const files = await utils.glob(path_1.join(namespaceRoot, '*', '*@(.app|.cmp|.intf|.evt|.lib)'), { cwd: namespaceRoot });
    return files;
    // const subdirs = await findSubdirectories(namespaceRoot);
    // for (const subdir of subdirs) {
    //     const basename = path.basename(subdir);
...
async function findCoreESLint() {
    // use highest version in ~/tools/eslint-tool/{version}
    const eslintToolDir = path.join(os_1.homedir(), 'tools', 'eslint-tool');
    ...
    for (const file of dirs) {
        ...
        if ((await fs.stat(subdir)).isDirectory()) {
            if (!highestVersion || semver_1.lt(highestVersion, file)) {
                highestVersion = file;
            }
        }
    }
