...
exports.glob = util_1.promisify(glob_1.Glob);
const RESOURCES_DIR = 'resources';
/**
 * @return true if changes include a directory delete
 */
function includesDeletedLwcWatchedDirectory(context, changes) {
    for (const event of changes) {
        if (event.type === vscode_languageserver_1.FileChangeType.Deleted && isLWCWatchedDirectory(context, event.uri)) {
            return true;
        }
    }
    return false;
...
function includesDeletedAuraWatchedDirectory(context, changes) {
    for (const event of changes) {
        if (event.type === vscode_languageserver_1.FileChangeType.Deleted && isAuraWatchedDirectory(context, event.uri)) {
            return true;
        }
    }
    return false;
...
function toResolvedPath(uri) {
    return path_1.resolve(vscode_uri_1.default.parse(uri).fsPath);
}
...
function getExtension(textDocument) {
    ...
    return filePath ? path_1.extname(filePath) : '';
}
exports.getExtension = getExtension;
function getResourcePath(resourceName) {
    return path_1.join(__dirname, RESOURCES_DIR, resourceName);
}
exports.getResourcePath = getResourcePath;
function getSfdxResource(resourceName) {
    return path_1.join(__dirname, RESOURCES_DIR, 'sfdx', resourceName);
}
exports.getSfdxResource = getSfdxResource;
function getCoreResource(resourceName) {
    return path_1.join(__dirname, RESOURCES_DIR, 'core', resourceName);
}
    ...
    for (const key of Object.keys(from)) {
        ...
        if (!to.hasOwnProperty(key)) {
            ...
            for (const e of fromVal) {
                if (!toArray.some(value => deep_equal_1.default(value, e))) {
                    toArray.push(e);
                    modified = true;
                }
            }
